...
- 기준일: 2026-08-11
- 현재 단계: **첫 Private Vertical Slice 승인 → Milestone 0 environment gate 진행 중**

이 파일은 문서를 찾기 위한 인덱스다. 제품 결정을 새로 만들지 않으며, 내용이 충돌할 때는 아래 source hierarchy를 따른다.

빠른 링크: [확정 결정](01_CONFIRMED_DECISIONS_AND_OPEN_GATES.md) · [첫 Slice 승인 기록](decisions/2026-08-12-first-private-slice-approval.md) · [상세 아키텍처](reports/architecture-decision-proposal.md) · [첫 Slice ExecPlan](plans/first-private-vertical-slice.md) · [ADR-0002](adr/0002-memory-local-domain-and-owner-boundary.md) · [ADR-0003 제안](adr/0003-android-image-intake-spike-toolchain.md) · [테스트 증거](reports/private-slice-test-evidence.md) · [단계 runbook](08_CODEX_PHASE_RUNBOOK.md)
...
| 4 | 승인된 `decisions/`, `adr/`, `plans/` | 해당 범위의 결정·기술 경계·실행 계획. `01`을 변경할 수 없음 |
...
| 9 | `docs/product/`, `docs/superpowers/`, 배포용 단일본/Word | legacy 또는 frozen snapshot. 현행 구현 지시로 사용하지 않음 |
...
| `06_ARCHITECTURE_AND_VERTICAL_SLICE_PLAN.md` | `GATED` | 전체 P0 아키텍처 제약과 slice chain. 첫 실행 slice의 상세 제안은 별도 report/ExecPlan 참조 |
| `07_QA_ANALYTICS_LAUNCH_OPERATIONS.md` | `CURRENT` | 테스트, privacy-safe analytics, 운영 기준. 베타 수치는 제안값 |
| `08_CODEX_PHASE_RUNBOOK.md` | `GATED` | 단계별 작업/승인 틀. local-first 순서가 제안됐으며 사용자 승인 전에는 실행하지 않음 |
...
| `plans/first-private-vertical-slice.md` | `APPROVED_IN_PROGRESS` | 구현 범위·마일스톤·검증·롤백 계획. Milestone 0 진행 중 |
...
세부 분류는 `../superpowers/README.md`를 따른다.
...
이 파일들을 현재 ExecPlan처럼 다시 실행하지 않는다.
...
| `/prompts/moemoa/02...` | `COMPLETED_APPROVED` | 상세 architecture proposal/ExecPlan 승인 완료. Milestone 0 진행 중 |
...
첫 slice와 `local-only Card/Archive → Board/Web → sync → private cloud → 제한 catalog` 순서는 승인됐다. 현재는 `adr/0003-android-image-intake-spike-toolchain.md`의 environment gate를 해결한다.

필요한 다음 확인:

1. 일반 project Node를 22+로 구성한다.
2. Android Studio를 2025.2.1+로 update한다.
3. spike application ID를 확인한다.
4. 이후 Capacitor 8.5 dependency 설치와 Android scaffold를 실행한다.
