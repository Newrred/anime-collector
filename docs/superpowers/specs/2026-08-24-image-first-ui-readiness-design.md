...
> **Status:** `CURRENT_APPROVED_DESIGN — IMPLEMENTATION NOT STARTED`
>
> **Approved:** 2026-08-24
>
...
- 한 화면에서 강한 primary action은 원칙적으로 하나만 둔다.
...
- 모바일 터치 target은 최소 44×44 CSS px이다.
...
- 320~479px는 2열, 480~899px는 2~3열, 900px 이상은 3~4열을 기본으로 한다.
...
fixture는 production 코드의 hidden bypass를 만들지 않는다. 기존 DEV/test seam과 IndexedDB repository의 public contract를 사용한다.

### 14.2 Screenshot 규칙

- Playwright `toHaveScreenshot` 또는 동등한 repository-local snapshot assertion을 사용한다.
- animation과 caret을 비활성화한다.
...
- pixel threshold를 높여 실패를 숨기지 않는다. 플랫폼 font rasterization 차이는 mask가 아니라 동일 bundled font와 Chromium pinning으로 줄인다.
...
- 모바일 primary touch target ≥ 44×44px
...
1. 공통 visual token, responsive container, button·surface 규칙과 screenshot harness.
...
- 네 승인 viewport의 screenshot baseline이 reviewer에게 승인된다.
