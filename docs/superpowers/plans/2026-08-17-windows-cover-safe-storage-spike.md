...
**Architecture:** The spike evaluated a hidden Windows PowerShell 5.1 broker with a C# `Add-Type` core, bounded binary protocol, no-follow component pinning, and no-replace immutable publication. Native x64 controls found no publication form that satisfies the approved constraints on this host: non-null `RootDirectory` relative forms return Win32 87, absolute forms return Win32 32 while the required pins are held, and null-root relative forms resolve against the process current directory. No backend is therefore accepted or connected; Windows cover persistence remains fail-closed.
...
## Blocked Result — 2026-08-17

- The earlier `RootDirectory + .\<digest>.<extension>` success was invalid evidence: the temporary oracle condition set `RootDirectory=NULL` whenever the name contained a separator, so the operation actually resolved against the process current directory.
- A corrected native x64 oracle forced non-null `RootDirectory` and returned Win32 87 for simple-leaf and dot-relative buffer/layout variants.
- `RootDirectory=NULL` plus a separator-free leaf also resolved against an external process-current-directory trap rather than the already-open source file's parent; collisions were likewise evaluated in that trap.
- No production route, Task 1 commit, Task 2 integration, dependency, binary, or weaker fallback is approved. The task steps below are retained as design history and must not be executed unless a new user-approved plan supplies a documented route and new acceptance gates.
...
- No final-publication form is presently permitted. In particular, prohibit non-null-root simple/dot-relative forms, null-root relative/current-directory forms, DOS/volume-GUID absolute targets, request-supplied paths, `MoveFileEx`, `CREATE_ALWAYS`, replacement rename, hard-link publication, and every retry/overwrite fallback unless a future approved plan replaces this blocked result with new evidence.
- Keep existing Windows production and fixture persistence at `COVER_STORAGE_PLATFORM_UNSAFE`; do not expose the partial broker/core or treat diagnostic files as a storage backend.
...
Add a current-directory trap test that changes the broker process current directory to a separate external directory before publication. The trap must remain empty, while the destination reopened through the retained final-parent path must have the same NTFS volume serial/file index, exact size, and SHA-256 as the still-open renamed handle. Add a test-only private-validator probe for `.\..\trap\x`, `..\x`, `x\y`, an absolute path, and every value other than exactly `.\<validated digest>.<validated extension>`; reject each before `SetFileInformationByHandle` without adding a production protocol command. Run the collision and eight-process convergence cases through this exact dot-relative form.
...
10. **BLOCKED — do not implement publication.** Corrected native x64 controls show Win32 87 for non-null-root simple and dot-relative forms; null-root absolute forms fail under the required pins; null-root relative forms publish into the process current directory. Preserve fail-closed behavior and require a separate approved plan before testing any different API, sharing mode, staging model, path resolution, or publication mechanism.
...
12. On success, retain the renamed handle, call `FlushFileBuffers` again, and reopen the destination no-follow while all pins remain held. Compare NTFS volume serial/file index to the renamed handle; require the renamed handle's normalized final path to equal the retained final-parent normalized path plus `<digest>.<extension>` at a component boundary; verify exact size and SHA-256; return `CREATED`; then close file, sentinel, and directory handles in reverse order.
...
Expected on Windows: PASS for protocol, missing helper, exact store/dedupe, dot-relative validation, current-directory trap isolation, final-path/file-identity verification, junctions, direct reparse mutation denial, lock pinning, collisions, cleanup, and eight-process concurrency. No absolute rename fallback is exercised. A symlink case may report SKIP only if the local token lacks capability.
...
Stop for independent review. If any test, leak gate, symlink-capable run, API/error review, or durability wording review fails, revert Task 2 or leave it unapproved; the safe state remains the Task 1-isolated/current Task 7 Windows fail-closed behavior. Task 8 remains blocked until both Task 1 and Task 2 have explicit passing review verdicts.
