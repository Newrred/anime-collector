...
- The final publication is same-directory and same-volume through `SetFileInformationByHandle(FileRenameInfo)` with `ReplaceIfExists=FALSE`, a retained final-parent `RootDirectory` handle, and the internally constructed relative path `.\<digest>.<extension>`. The portion after the required `.\` prefix is the already validated fixed checksum leaf: no `..`, additional separator, arbitrary relative path, request-supplied path, process-current-directory resolution, DOS/volume-GUID absolute target, `MoveFileEx`, `CREATE_ALWAYS`, replacement rename, hard-link publication, or overwrite/absolute retry is permitted.
- Keep the entire `FILE_SHARE_READ`-only pin chain and temp handle open through rename. Reopen the destination no-follow, compare NTFS volume/file identity with the still-open renamed handle, require its normalized final path to equal the retained final-parent path plus the fixed leaf at a component boundary, and recompute exact size and SHA-256 before success.
...
10. Allocate architecture-correct `FILE_RENAME_INFO` memory with `ReplaceIfExists=FALSE`. The temporary x64 Windows-SDK oracle outside the repository has established that a simple leaf returns Win32 87 while a non-null retained final-parent `RootDirectory` plus `.\<digest>.<extension>` succeeds on this host. Construct that exact dot-relative value internally from the separately validated digest and extension, revalidate that it contains no other separator or traversal component, and call `SetFileInformationByHandle(FileRenameInfo)` once. Do not retain the probe binary, accept any request path spelling, resolve against the process current directory, or retry using a null root or absolute target.
