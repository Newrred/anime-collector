...
**Architecture:** A hidden Windows PowerShell 5.1 broker loads one tracked C# source file with `Add-Type`, then serves a bounded binary protocol over anonymous stdin/stdout pipes. The C# core pins every path component with documented Win32 handles, rejects all reparse points, publishes a fully flushed temporary file with handle-relative `FileRenameInfo` and `ReplaceIfExists=FALSE`, and verifies every collision; Node integrates it only after the isolated backend passes Windows review.
...
- Every directory/file component is opened with `FILE_FLAG_OPEN_REPARSE_POINT`; every reparse point is rejected; validated handles stay open while descending and mutating; `FILE_SHARE_DELETE` is never granted on pinned components.
- The final publication is same-directory and same-volume through `SetFileInformationByHandle(FileRenameInfo)` with a pinned `RootDirectory`, a simple relative checksum filename, and `ReplaceIfExists=FALSE`. Do not fall back to a path-only overwrite, `CREATE_ALWAYS`, replacement rename, or hard-link publication.
...
10. Allocate architecture-correct `FILE_RENAME_INFO` memory with `ReplaceIfExists=FALSE`, `RootDirectory` equal to the pinned final-directory handle, and relative `<digest>.<extension>`. Call `SetFileInformationByHandle(FileRenameInfo)`.
...
12. On success, retain the renamed handle, call `FlushFileBuffers` again, verify attributes/size, return `CREATED`, then close file, sentinel, and directory handles in reverse order.
