...
- 빈 보관함 상태에는 작품 추가를 위한 primary CTA를 하나만 둔다. 같은 목적의 보조 버튼이나 링크로 CTA를 중복하지 않는다.
- 감상 로그 작성 화면을 열거나 닫는 행동만으로 데이터가 변경되면 안 된다. 새 로그와 수정 사항은 사용자가 `Save`를 선택한 뒤에만 영속화한다.
- 전용 탐색은 `Home / Library / Tier`를 기본으로 유지한다. 기존 사용자의 저장 언어 설정은 유지하되, 새 방문자의 기본 문구는 영어로 시작한다.
- 클라우드 상태 문구는 실제 구성·연결·확인 결과보다 앞서 나가면 안 된다. 확인되지 않은 백업, 동기화 완료, 원격 데이터의 존재를 암시하지 않는다.
