...
export async function prepareSelectedPublicImage({ sourceAssetId,sourceVersion,operationId,accessToken,policyRevision,
  consented=false,readOriginal,signal,fetchImpl=globalThis.fetch }) {
  ...
  if(typeof readOriginal!=="function") fail("ORIGINAL_IMAGE_UNAVAILABLE");
  let original;
  ...
  try {
    response=await fetchImpl("/api/public-image",{method:"POST",signal,cache:"no-store",
      headers:{"Content-Type":"application/octet-stream",Authorization:`Bearer ${accessToken}`,
        "X-Moemoa-Asset":sourceAssetId,"X-Moemoa-Version":String(sourceVersion),"X-Moemoa-Operation":operationId,"X-Moemoa-Consent":policyRevision},body:original});
