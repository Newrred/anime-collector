...
select has_table('public', 'catalog_cover_revisions', 'durable catalog cover revision table exists');
...
select has_column('public', 'memory_visual_assets', 'catalog_cover_id', 'visual assets can reference a stable catalog cover');
select has_column('public', 'memory_visual_assets', 'catalog_cover_revision_id', 'visual assets can pin an immutable cover revision');
select has_column('public', 'memory_visual_assets', 'catalog_anime_id', 'cover reference retains its catalog anime identity');
select has_column('public', 'memory_visual_assets', 'permission_verified_at', 'cover reference retains permission verification time');
...
select has_index('public', 'memory_board_cards', 'memory_board_cards_owner_card_idx', 'board membership card foreign key is covered');
select has_index('public', 'memory_visual_assets', 'memory_visual_assets_owner_card_idx', 'visual asset card foreign key is covered');
select has_index('public', 'sync_operations', 'sync_operations_owner_device_idx', 'sync operation device foreign key is covered');
select has_index('public', 'user_account_promotions', 'user_account_promotions_owner_device_idx', 'promotion device foreign key is covered');
