...
async function createBoard(page, title: string) {
  ...
  await expect(page.locator(".memory-boards__title-input")).toHaveCount(0);
}

async function addCard(page, title: string) {
  const tools = page.locator("details").filter({ has: page.locator(".memory-boards__add") });
  if (!(await tools.getAttribute("open"))) {
    if (!(await page.locator(".memory-boards__add").isVisible())) await tools.locator("summary").click();
  }
...
test("private Boards keep cards reusable, ordered, and independent from Archive", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Move Alpha memory down" })).toHaveCount(0);
...
test("deleting a Card tombstones its Board membership without deleting the Board", async ({ page }) => {
  ...
  await expect(page).toHaveURL(sourceBoardUrl);
